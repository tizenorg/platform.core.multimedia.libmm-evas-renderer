...
void _flush_packets(mm_evas_info *evas_info);
...
int _mm_evas_renderer_retrieve_all_packets(mm_evas_info *evas_info, int keep_screen);
...
int _mm_evas_renderer_retrieve_all_packets(mm_evas_info *evas_info, int keep_screen)
{
	...
	if (pid==tid) {
		...
		LOGD("[%d] it might be main thread", pid);
		_flush_packets(evas_info);
	} else {
