...
void
_evas_pipe_cb (void *data, void *buffer, update_info info)
{
	mm_evas_info *evas_info = data;
	int ret;
...
static void
_mm_evas_renderer_set_callback (mm_evas_info * evas_info)
{
	if(evas_info->eo)
	{
...
static void
_mm_evas_renderer_unset_callback (mm_evas_info * evas_info)
{
	if(evas_info->eo)
	{
...
int
_mm_evas_renderer_create (mm_evas_info ** evas_info)
{
...
int
_mm_evas_renderer_destroy (mm_evas_info * evas_info)
{
	MMPLAYER_CHECK_NULL(evas_info);

	LOGD ("finalize evas_info");

	_mm_evas_renderer_reset(evas_info);
...
int
_mm_evas_renderer_set_info (mm_evas_info * evas_info, Evas_Object *eo)
{
	MMPLAYER_CHECK_NULL(evas_info);
...
int
_mm_evas_renderer_reset (mm_evas_info * evas_info)
{
	MMPLAYER_CHECK_NULL(evas_info);
...
void
_mm_evas_renderer_update_geometry (mm_evas_info *evas_info, rect_info *result)
{
	if (!evas_info || !evas_info->eo) {
...
int
_mm_evas_renderer_apply_geometry (mm_evas_info *evas_info)
{
	if (!evas_info || !evas_info->eo) {
		LOGW("there is no evas_info or evas object");
		return MM_ERROR_NONE;
